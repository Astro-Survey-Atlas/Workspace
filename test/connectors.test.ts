...
test("connector registry persists S3, local, and JDBC configuration without testing connections", async () => {
  ...
  try {
    ...
    const reloaded = new ConnectorRegistry(path.join(directory, "connectors.json"));
    await reloaded.initialize();
    assert.equal(reloaded.get(s3.id).config.bucket, "euclid");
...
test("connector registry seeds a new state file from the bundled bootstrap", async () => {
  ...
  try {
    ...
    const registry = new ConnectorRegistry(path.join(directory, "state", "connectors.json"), bootstrapPath);
    await registry.initialize();
    assert.equal(registry.get("connector-fixture").config.bucket, "fixture");
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("concurrent connector registrations retain both records", async () => {
  ...
  try {
    ...
    const registry = new ConnectorRegistry(statePath);
    await registry.initialize();
    await Promise.all([
    ...
    const reloaded = new ConnectorRegistry(statePath);
    await reloaded.initialize();
...
test("bootstrap connector ids remain aliases after a path-based state migration", async () => {
  ...
  try {
    ...
    const registry = new ConnectorRegistry(statePath, bootstrapPath);
    await registry.initialize();
    assert.equal(registry.get("connector-legacy").id, "connector-current");
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test("S3 checks distinguish endpoint reachability from transient credential verification", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(persisted, /temporary-access|temporary-secret/);
  } finally {
    server.close();
    await rm(directory, { recursive: true, force: true });
  }
