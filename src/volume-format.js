...
function alignedTargetIdOffset(count) {
...
export function expectedVolumeByteLength(count) {
    return alignedTargetIdOffset(count) + count * BigUint64Array.BYTES_PER_ELEMENT;
}
export function decodeVolumePoints(buffer, expectedCount) {
    if (buffer.byteLength < VOLUME_HEADER_BYTES)
        throw new Error("Volume binary is shorter than its header");
    const bytes = new Uint8Array(buffer, 0, VOLUME_MAGIC.length);
    const magic = String.fromCharCode(...bytes);
    if (magic !== VOLUME_MAGIC)
        throw new Error(`Unsupported volume magic: ${magic}`);
    ...
    if (version !== VOLUME_FORMAT_VERSION)
        throw new Error(`Unsupported volume format version: ${version}`);
