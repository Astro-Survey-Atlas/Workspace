...
const MODALITIES = ["imaging", "spectroscopy", "photometry", "time-domain", "integral-field", "ultraviolet", "infrared", "catalog"];
function release(id, label, options) {
    return { id, label, ...options };
}
const sdssReleases = Array.from({ length: 19 }, (_, index) => {
    const number = index + 1;
    const phase = number <= 7 ? "SDSS-I/II" : number <= 12 ? "SDSS-III" : number <= 17 ? "SDSS-IV" : "SDSS-V";
    const modalities = number <= 7
    ...
    const coverage = number === 7
        ? { status: "summary_only", summary: "DR7 imaging catalog: 11,663 deg2 total; unique Legacy imaging: 8,423 deg2; spectroscopy: 9,380 deg2. Product footprints differ.", areaDeg2: 11663, sourceUrl: "https://www.sdss.org/dr7/" }
        : number === 17
            ? { status: "summary_only", summary: "Final SDSS-IV release. Imaging, optical spectra, APOGEE, MaNGA and MaStar use distinct footprints.", sourceUrl: "https://www.sdss.org/dr17/" }
            : { status: "pending", summary: "Release is registered; product-level footprint artifact has not yet been ingested.", sourceUrl: `https://www.sdss.org/dr${number}/` };
...
const legacyReleases = Array.from({ length: 10 }, (_, index) => {
    const number = index + 1;
...
function deriveCoverageStatus(releases) {
    if (releases.some((entry) => entry.coverage.status === "verified"))
        return "verified";
    if (releases.some((entry) => entry.coverage.status === "summary_only"))
        return "summary_only";
    return "pending";
}
...
function validateRegistration(input) {
    ...
    if (!name || name.length > 120)
        throw new RangeError("name must contain 1 to 120 characters");
    if (!sourceUrl || sourceUrl.length > 2048)
        throw new RangeError("sourceUrl must contain 1 to 2048 characters");
    let protocol;
    try {
    ...
    if (protocol !== "https:" && protocol !== "http:")
        throw new RangeError("sourceUrl must use http or https");
    ...
    if (description && description.length > 500)
        throw new RangeError("description must contain at most 500 characters");
    ...
    async initialize() {
        try {
            ...
            if (!Array.isArray(parsed))
                throw new Error("survey registry state must be an array");
            this.#registrations = parsed.filter((entry) => Boolean(entry) && typeof entry === "object" && entry.origin === "user");
        }
        catch (error) {
