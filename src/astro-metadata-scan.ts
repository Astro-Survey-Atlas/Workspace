...
export interface AstroMetadataScanTask { apiVersion?: string; kind?: "AstroMetadataScanTask"; metadata?: { name?: string; namespace?: string; labels?: Record<string, string> }; spec: { backend?: ScanBackend; source: { dataSourceRef?: ScanDataSourceRef; paths: string[] }; tags?: string[]; userProperties?: Record<string, string>; fileNamePattern?: string; pathPatterns?: Record<string, string>; handlers?: string[]; sink?: { dataSourceRef: ScanDataSourceRef }; extraEnv?: Record<string, string> } }
export interface ScanExecutionPlan { apiVersion: string; taskName: string; backend: ScanBackend; source: { dataSourceRef?: ScanDataSourceRef; paths: string[] }; tags: string[]; userProperties: Record<string, string>; fileNamePattern?: string; pathPatterns: Record<string, string>; handlers: HandlerName[]; sink?: { dataSourceRef: ScanDataSourceRef }; extraEnv: Record<string, string> }
...
function basenamePattern(value: unknown): string | undefined {
  if (value === undefined) return undefined;
  const pattern = text(value, "spec.fileNamePattern", 512);
  if (/[\/\0\r\n]/.test(pattern)) throw new RangeError("spec.fileNamePattern must match a basename");
  try { const compiled = new RegExp(pattern); if (compiled.test("a/b") || compiled.test("a\\b")) throw new Error(); }
  catch { throw new RangeError("spec.fileNamePattern must be a safe basename regular expression"); }
  return pattern;
}
...
export function validateAstroMetadataScanTask(input: unknown): AstroMetadataScanTask {
  ...
  const fileNamePattern = basenamePattern(spec.fileNamePattern);
  ...
  return structuredClone({ ...value, apiVersion: ASTRO_METADATA_API, kind: "AstroMetadataScanTask", spec: { ...spec, backend, source: { ...source, paths }, handlers, tags, userProperties, ...(fileNamePattern ? { fileNamePattern } : {}), pathPatterns, extraEnv } }) as AstroMetadataScanTask;
}
...
export function compileScanExecutionPlan(taskInput: unknown, dataSource?: AstroDataSource): ScanExecutionPlan {
  ...
  return { apiVersion: ASTRO_METADATA_API, taskName: name(task.metadata?.name ?? "scan", "metadata.name"), backend: task.spec.backend ?? "job", source: structuredClone(task.spec.source), tags: [...(task.spec.tags ?? [])], userProperties: { ...(task.spec.userProperties ?? {}) }, ...(task.spec.fileNamePattern ? { fileNamePattern: task.spec.fileNamePattern } : {}), pathPatterns: { ...(task.spec.pathPatterns ?? {}) }, handlers: [...(task.spec.handlers ?? ["default", "fits", "coverage"])] as HandlerName[], ...(task.spec.sink ? { sink: structuredClone(task.spec.sink) } : {}), extraEnv: { ...(task.spec.extraEnv ?? {}) } };
}
