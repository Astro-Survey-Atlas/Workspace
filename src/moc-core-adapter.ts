...
export class MocCoreCliAdapter implements MocCoreAdapter {
  ...
  constructor(command = process.env.ASTRO_MOC_CORE_CLI ?? "astro-survey-assets", timeoutMs = 120_000) {
    this.#command = command.trim() || "astro-survey-assets";
  ...
  async buildCatalog(input: MocCoreCatalogInput): Promise<MocCoreCatalogResult> {
    ...
    try {
      ...
      try {
        ...
        ({ stdout } = await execFileAsync(program!, [...prefixArgs, "build", "--spec", specPath, "--output", outputPath, "--base-dir", "/"], {
