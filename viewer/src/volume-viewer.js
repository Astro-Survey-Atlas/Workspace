...
function asVector(raDeg, decDeg, radius = 1) {
    const point = raDecToCartesian(raDeg, decDeg, radius);
    return new THREE.Vector3(point.x, point.y, point.z);
}
function shellGridGeometry(radius, phiStart, phiLength, dense) {
    const positions = [];
    const appendSegment = (start, end) => {
        positions.push(start.x, start.y, start.z, end.x, end.y, end.z);
    };
    const point = (phi, theta) => new THREE.Vector3(-radius * Math.cos(phi) * Math.sin(theta), radius * Math.cos(theta), radius * Math.sin(phi) * Math.sin(theta));
    ...
    for (let latitude = 1; latitude < latitudeCount; latitude += 1) {
        ...
        for (let step = 0; step < arcSteps; step += 1) {
            appendSegment(point(phiStart + (phiLength * step) / arcSteps, theta), point(phiStart + (phiLength * (step + 1)) / arcSteps, theta));
        }
    }
    return new THREE.BufferGeometry().setAttribute("position", new THREE.Float32BufferAttribute(positions, 3));
}
function radialGuideGeometry(phiStart, phiLength) {
    const positions = [];
    ...
    directions.forEach((phi) => {
        for (let latitude = 0; latitude <= 6; latitude += 1) {
            ...
            positions.push(0, 0, 0, -Math.cos(phi) * Math.sin(theta), Math.cos(theta), Math.sin(phi) * Math.sin(theta));
        }
    });
    return new THREE.BufferGeometry().setAttribute("position", new THREE.Float32BufferAttribute(positions, 3));
}
export class VolumeViewer {
    ...
    #camera = new THREE.PerspectiveCamera(52, 1, 0.015, 20);
    #renderer;
    #controls;
    #raycaster = new THREE.Raycaster();
    #pointer = new THREE.Vector2();
    #shellLayer = new THREE.Group();
    #dataLayer = new THREE.Group();
    #voxelLayer = new THREE.Group();
    #selectionLayer = new THREE.Group();
    #resizeObserver;
    #cutRotation = new THREE.Quaternion();
    #onSelection;
    #onStateChange;
    #points = null;
    #pointCloud = null;
    ...
    #renderQueued = false;
    #pointerStart = null;
    constructor(canvas, manifest, onSelection, onStateChange) {
        this.#canvas = canvas;
    ...
    setData(points) {
        if (points.count !== this.#manifest.pointCount)
            throw new Error("Point data does not match volume manifest");
        this.#points = points;
        if (this.#pointCloud) {
        ...
        for (let index = 0; index < points.count; index += 1) {
            ...
            const position = volumePosition(points.raDeg[index], points.decDeg[index], distanceMpc, this.#manifest.radialCoordinate.domainMaxMpc, OUTER_RADIUS);
            ...
            angularVisible[index] = !filterHealpix || filterHealpix.ang2pix(new Pointing(null, false, ((90 - points.decDeg[index]) * Math.PI) / 180, (points.raDeg[index] * Math.PI) / 180)) === this.#angularFilter.pixel ? 1 : 0;
        }
    ...
    setAngularFilter(filter) {
        this.#angularFilter = filter;
        if (!this.#points || !this.#pointCloud)
            return;
        const attribute = this.#pointCloud.geometry.getAttribute("aAngularVisible");
        const filterHealpix = filter ? new Healpix(filter.nside) : null;
        for (let index = 0; index < this.#points.count; index += 1) {
            const visible = !filterHealpix || filterHealpix.ang2pix(new Pointing(null, false, ((90 - this.#points.decDeg[index]) * Math.PI) / 180, (this.#points.raDeg[index] * Math.PI) / 180)) === filter.pixel;
    ...
    setJointCells(cells, nside, radialBins) {
    ...
    setRepresentation(representation) {
        ...
        if (this.#voxelMesh)
            this.#voxelMesh.visible = representation === "cells";
        if (this.#pointCloud)
            this.#pointCloud.visible = representation === "points";
    ...
    setShellCount(shellCount) {
        if (![1, 2, 4, 8, 16, 32].includes(shellCount))
            throw new RangeError("Unsupported shell count");
        this.#shellCount = shellCount;
        this.#clearSelection();
        this.#rebuildShells();
    }
    setCutAngle(cutAngleDeg) {
    ...
    setPointSize(size) {
        ...
        if (this.#pointMaterial)
            this.#pointMaterial.uniforms.uPointSize.value = this.#pointSize * Math.min(window.devicePixelRatio, 2);
        this.#emitState();
        this.#requestRender();
    }
    setRadialRange(minMpc, maxMpc) {
    ...
    #rebuildShells() {
        ...
        for (let index = 1; index <= this.#shellCount; index += 1) {
            ...
            const surface = new THREE.Mesh(new THREE.SphereGeometry(radius, SHELL_SEGMENTS, 36, phiStart, phiLength, 0, Math.PI), new THREE.MeshBasicMaterial({
                color,
                transparent: true,
            ...
            const grid = new THREE.LineSegments(shellGridGeometry(radius, phiStart, phiLength, index === this.#shellCount), new THREE.LineBasicMaterial({
                color,
                transparent: true,
        ...
        const guides = new THREE.LineSegments(radialGuideGeometry(phiStart, phiLength), new THREE.LineBasicMaterial({ color: 0xb7c0c9, transparent: true, opacity: 0.35, depthWrite: false }));
        guides.renderOrder = this.#shellCount + 2;
        this.#shellLayer.add(guides);
        const origin = new THREE.Mesh(new THREE.SphereGeometry(0.012, 12, 8), new THREE.MeshBasicMaterial({ color: 0xf4f7f8 }));
    ...
    #pick(event) {
        ...
        if (this.#representation === "cells" && this.#voxelMesh) {
            ...
            const domainMaxMpc = this.#manifest.radialCoordinate.domainMaxMpc;
            const marker = new THREE.Mesh(buildSphericalCellGeometry([{
        ...
        if (!this.#pointCloud || !this.#points)
            return;
        ...
        const marker = new THREE.Mesh(new THREE.SphereGeometry(0.018, 16, 10), new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.96 }));
