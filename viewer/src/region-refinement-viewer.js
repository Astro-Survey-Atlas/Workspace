...
import { buildSphericalCellEdges, buildSphericalCellSheetGeometry, healpixPixelFromSceneDirection, sphericalCellBoundary, } from "./spherical-cell-geometry";
...
function childrenOf(pixels) {
    return [...pixels].flatMap((pixel) => [pixel * 4, pixel * 4 + 1, pixel * 4 + 2, pixel * 4 + 3]);
}
export function ancestorPixel(pixel, nside, ancestorNside) {
    if (ancestorNside > nside || nside % ancestorNside !== 0)
        throw new RangeError("ancestor NSIDE must divide the current NSIDE");
...
export class RegionRefinementViewer {
    ...
    #onStateChange;
    #history = [];
    #coveredBasePixels = new Set();
    #nside;
    #candidatePixels;
    #selectedPixels;
    #pointerStart = null;
    #renderQueued = false;
    constructor(canvas, baseNside, basePixels, coveredBasePixels, onStateChange) {
        if (!basePixels.length)
            throw new RangeError("A region is required for refinement");
        this.#canvas = canvas;
    ...
    setCoveredBasePixels(pixels, rebuild = true) {
        this.#coveredBasePixels.clear();
        for (const pixel of pixels)
            this.#coveredBasePixels.add(pixel);
        if (rebuild)
            this.#rebuild();
    }
    refine() {
        if (this.#nside >= MAX_NSIDE || !this.#selectedPixels.size)
            return;
        this.#history.push({
            nside: this.#nside,
            candidatePixels: [...this.#candidatePixels],
    ...
    #toggle(pixel) {
        if (this.#selectedPixels.has(pixel))
            this.#selectedPixels.delete(pixel);
        else
            this.#selectedPixels.add(pixel);
        this.#rebuild();
    }
    #focusRegion() {
    ...
    #handleControlsChange = () => this.#requestRender();
