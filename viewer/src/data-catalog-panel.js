...
function projectStatesLabel(asset) {
...
function connectorLocation(record) {
    const config = record.config;
    if (record.kind === "local")
        return config.rootPath ?? "本地路径未配置";
    if (record.kind === "s3")
        return `${config.endpoint ? `${config.endpoint.replace(/\/$/, "")}/` : "s3://"}${config.bucket ?? ""}${config.prefix ? `/${config.prefix.replace(/^\//, "")}` : ""}`;
...
export class DataCatalogPanel {
    ...
    #detailOpen = false;
    #formEditable = false;
    constructor(onError, onConnectorSelected) {
        this.#onError = onError;
        this.#onConnectorSelected = onConnectorSelected;
        byId("catalog-search").addEventListener("input", () => this.#render());
        byId("catalog-origin-filter").addEventListener("change", () => this.#render());
        byId("catalog-kind-filter").addEventListener("change", () => this.#render());
        byId("catalog-survey").addEventListener("change", () => this.#syncReleaseOptions());
        byId("catalog-registration-form").addEventListener("submit", (event) => {
            event.preventDefault();
            void this.#save().catch(this.#onError);
        });
        byId("catalog-form-cancel").addEventListener("click", () => this.#resetForm());
        byId("catalog-new").addEventListener("click", () => this.#startNew());
        byId("catalog-form-edit").addEventListener("click", () => {
            const asset = this.#assets.find((candidate) => candidate.id === this.#selectedId);
            if (asset)
                this.#beginEdit(asset);
        });
        byId("catalog-delete").addEventListener("click", () => void this.#remove().catch(this.#onError));
        byId("asset-detail-back").addEventListener("click", () => this.#closeDetail());
    }
    async activate(surveys, records) {
        this.#active = true;
    ...
    #filteredAssets() {
        ...
        const origin = byId("catalog-origin-filter").value;
        const kind = byId("catalog-kind-filter").value;
        return this.#assets.filter((asset) => {
            if (origin !== "all" && asset.origin !== origin)
                return false;
    ...
    #setFormDisabled(disabled) {
        ...
        byId("catalog-registration-form").querySelectorAll("input, select, textarea").forEach((field) => {
            field.disabled = disabled;
        });
        byId("catalog-form-edit").hidden = !disabled;
        byId("catalog-form-submit").hidden = disabled;
        byId("catalog-form-cancel").hidden = disabled;
    }
    #renderBasicForm(asset) {
        if (this.#formEditable)
            return;
        if (!asset) {
            byId("catalog-registration-form").reset();
            byId("catalog-form-title").textContent = "登记用户数据";
            this.#setFormDisabled(false);
            byId("catalog-form-edit").hidden = true;
            byId("catalog-form-cancel").hidden = true;
            byId("catalog-form-submit").hidden = false;
            return;
        }
        byId("catalog-name").value = asset.name;
        byId("catalog-description").value = asset.description;
        byId("catalog-survey").value = asset.surveyId ?? "";
        this.#syncReleaseOptions();
        byId("catalog-release").value = asset.releaseId ?? "";
        byId("catalog-product").value = asset.product;
        byId("catalog-kind").value = asset.kind;
        byId("catalog-status-input").value = asset.status;
    ...
    #renderInspector() {
        ...
        const summaryItems = [["状态", statusLabel(asset.status)], ["访问", this.#resolvedAccesses(asset).map((access) => access.uri).join(" · ")]];
    ...
    #renderSurveyOptions() {
        const select = byId("catalog-survey");
    ...
    #syncReleaseOptions() {
        const surveyId = byId("catalog-survey").value;
        const select = byId("catalog-release");
        const current = select.value;
    ...
    #surveyName(id) { return id ? this.#surveys.find((survey) => survey.id === id)?.name ?? id : "独立数据"; }
    #tagLabel(id) { return this.#tags.find((tag) => tag.id === id)?.label ?? id; }
    #tagText(tags) { return tags.map((tag) => this.#tagLabel(tag)).join(" · "); }
    #renderTags(selected = []) {
    ...
    #selectedTags() { return [...byId("catalog-tags").querySelectorAll("input:checked")].map((input) => input.value); }
    #input() {
        return {
            name: inputValue("catalog-name"), description: inputValue("catalog-description") || undefined,
            surveyId: byId("catalog-survey").value || undefined,
            releaseId: byId("catalog-release").value || undefined,
            product: inputValue("catalog-product") || undefined,
            kind: byId("catalog-kind").value,
            ...
            status: byId("catalog-status-input").value,
    ...
    #beginEdit(asset) {
        ...
        byId("catalog-delete").hidden = asset.origin === "builtin";
        this.#setFormDisabled(false);
    ...
    #startNew() {
        ...
        byId("catalog-registration-form").reset();
        this.#renderSurveyOptions();
        byId("catalog-kind").value = "catalog";
        byId("catalog-status-input").value = "metadata_only";
    ...
    #openDetail(asset) { this.#detailOpen = true; this.#detailEditing = null; byId("catalog-stage").hidden = true; byId("asset-detail-stage").hidden = false; this.#renderDetailPage(); }
    #closeDetail() { this.#detailOpen = false; this.#detailEditing = null; byId("catalog-stage").hidden = false; byId("asset-detail-stage").hidden = true; }
    #detailSection(title, content, action, callback) {
        const wrapper = document.createElement("section");
        wrapper.className = "asset-detail-section";
        const header = document.createElement("div");
        header.className = "asset-detail-section-heading";
    ...
    #basicEditor(asset) {
        ...
        form.elements.namedItem("description").value = asset.description;
        form.elements.namedItem("product").value = asset.product;
        form.elements.namedItem("kind").value = asset.kind;
        form.addEventListener("submit", (event) => { event.preventDefault(); const data = new FormData(form); this.#saveDetail(this.#assetInput(asset, { name: String(data.get("name") ?? "").trim(), description: String(data.get("description") ?? "").trim(), product: String(data.get("product") ?? "").trim(), kind: String(data.get("kind")) })); });
        form.querySelector("[data-cancel]")?.addEventListener("click", () => { this.#detailEditing = null; this.#renderDetailPage(); });
        return form;
    }
    #sourceEditor(asset) {
        const form = document.createElement("form");
        form.className = "detail-editor source-detail-editor";
        ...
        form.addEventListener("submit", (event) => { event.preventDefault(); const sources = [...form.querySelectorAll(".detail-source-row")].map((row) => ({ label: row.querySelector(".source-label").value.trim(), url: row.querySelector(".source-url").value.trim(), description: row.querySelector(".source-description").value.trim() || undefined })).filter((source) => source.label && source.url); this.#saveDetail(this.#assetInput(asset, { sources })); });
        form.querySelector("[data-cancel]")?.addEventListener("click", () => { this.#detailEditing = null; this.#renderDetailPage(); });
        return form;
    }
    #accessEditor(asset) {
        const form = document.createElement("form");
        form.className = "detail-editor connector-picker";
        ...
        const selected = new Set(asset.connectorLocationKeys?.length ? asset.connectorLocationKeys : this.#connectorRecordsFor(asset).map((connector) => connector.locationKey));
        form.querySelectorAll("input[type=checkbox]").forEach((input) => { input.checked = selected.has(input.value); });
        const actions = document.createElement("div");
        actions.className = "detail-editor-actions";
        actions.innerHTML = `<button class="command-button" type="submit">保存关联</button><button class="text-button" type="button" data-cancel>取消</button>`;
        form.append(actions);
        form.addEventListener("submit", (event) => { event.preventDefault(); const connectorLocationKeys = [...form.querySelectorAll("input:checked")].map((input) => input.value); const connectorIds = this.#connectors.filter((connector) => connectorLocationKeys.includes(connector.locationKey)).map((connector) => connector.id); this.#saveDetail(this.#assetInput(asset, { connectorIds, connectorLocationKeys })); });
        form.querySelector("[data-cancel]")?.addEventListener("click", () => { this.#detailEditing = null; this.#renderDetailPage(); });
        return form;
    }
    #lineageEditor(asset) {
        ...
        const addRow = (entry = { relation: "derived_from", label: "" }) => {
            const row = document.createElement("div");
        ...
        form.addEventListener("submit", (event) => {
            event.preventDefault();
            const lineage = [...rows.querySelectorAll(".lineage-editor-row")].map((row) => ({
                relation: row.querySelector(".lineage-relation").value.trim(),
                label: row.querySelector(".lineage-label").value.trim(),
                assetId: row.querySelector(".lineage-asset-id").value.trim() || undefined,
    ...
    #renderDetailPage() {
        ...
        if (!asset)
            return;
        byId("asset-detail-title").textContent = asset.name;
        byId("asset-detail-description").textContent = asset.description;
        const stats = byId("asset-detail-stats");
        stats.replaceChildren();
        const statItems = [["状态", statusLabel(asset.status)], ["Tag", String((asset.tags ?? asset.modalities).length)], ["Connector", String(this.#connectorRecordsFor(asset).length)], ["文件", "待扫描"]];
        statItems.forEach(([label, value]) => { const row = document.createElement("div"); const dt = document.createElement("dt"); const dd = document.createElement("dd"); dt.textContent = label; dd.textContent = value; row.append(dt, dd); stats.append(row); });
        ...
        if (this.#detailEditing !== "sources") {
            (asset.sources ?? []).forEach((source) => { const item = document.createElement("li"); const link = document.createElement("a"); link.href = source.url; link.target = "_blank"; link.rel = "noreferrer"; link.textContent = `${source.label}: ${source.url}`; item.append(link); source.description && item.append(` · ${source.description}`); sourceList.append(item); });
            if (!sourceList.childElementCount)
                sourceList.textContent = "尚未登记公开来源";
        }
        ...
        if (this.#detailEditing !== "access") {
            this.#resolvedAccesses(asset).forEach((access) => {
                ...
                if (access.connectorId && this.#connectors.some((connector) => connector.id === access.connectorId)) {
                    ...
                    link.type = "button";
                    link.className = "access-connector-link";
                    link.textContent = `${access.label ? `${access.label} · ` : ""}${access.connector} · ${access.uri} · ${access.format}`;
                    link.addEventListener("click", () => this.#onConnectorSelected?.(access.connectorId));
                    item.append(link);
                }
                else
                    item.textContent = `${access.label ? `${access.label} · ` : ""}${access.connector} · ${access.uri} · ${access.format}`;
                accessList.append(item);
            });
        }
        const lineage = this.#detailEditing === "lineage" ? this.#lineageEditor(asset) : document.createElement("div");
        lineage.className = "lineage-detail-tree";
        if (this.#detailEditing !== "lineage") {
            (asset.lineage ?? []).forEach((entry) => { const item = document.createElement("div"); item.textContent = `${entry.relation} · ${entry.label}`; lineage.append(item); });
            if (!lineage.childElementCount)
                lineage.textContent = "暂无血缘关系。交叉匹配、Cutout 或打包任务完成后，这里会记录来源资产和派生结果。";
        }
        const body = byId("asset-detail-body");
        body.replaceChildren(this.#detailSection("基本信息", basic, "编辑基本信息", () => { this.#detailEditing = "basic"; this.#renderDetailPage(); }), this.#detailSection("公开来源", sourceList, "编辑公开来源", () => { this.#detailEditing = "sources"; this.#renderDetailPage(); }), this.#detailSection("访问位置", accessList, "编辑 Connector 关联", () => { this.#detailEditing = "access"; this.#renderDetailPage(); }), this.#detailSection("数据血缘", lineage, "编辑血缘关系", () => { this.#detailEditing = "lineage"; this.#renderDetailPage(); }));
    }
    #setProjectStates(states) { byId("catalog-project-state-list").querySelectorAll("input[type=checkbox]").forEach((input) => { input.checked = states.includes(input.value); }); }
    #readProjectStates() { const states = [...byId("catalog-project-state-list").querySelectorAll("input:checked")].map((input) => input.value); return states.length ? states : ["planned"]; }
}
