...
export class SurveyLayerViewer {
  ...
  constructor(
    ...
    surveys.forEach((survey) => this.#colorBySurvey.set(survey.id, new THREE.Color(surveyDisplayColor(survey.id, survey.color))));
  ...
  setActiveOverlapComponent(componentId: string | null): void {
    ...
    if (this.#overlapMode && componentId) {
      ...
      if (component) {
        ...
        const radius = 1;
        const cells = component.cells.map((pixel) => ({ nside, pixel, radius, color: SELECTION_COLOR, inset: nside === this.#manifest.nside ? 0.018 : 0.006 }));
  ...
  #buildOverlapLayer(animated: boolean): void {
    ...
    const radius = 1;
    const cells = pixels.map((pixel) => ({ nside, pixel, radius, color: this.#overlapColor(), inset: nside === this.#manifest.nside ? 0.018 : 0.006 }));
  ...
  #rebuildOverlapLabels(nside = this.#overlapNside ?? this.#manifest.nside, pixels = this.#overlapPixels ?? [], radius = 1): void {
    clearGroup(this.#overlapLabelGroup);
  ...
  #addFragmentLayer(surveyId: string, pixels: number[], cells: SphericalCellSheetGeometryInput[], animated: boolean, renderOrder = 4): void {
    ...
    if (isOverlap) {
      ...
      const glowMaterial = new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: animated ? 0 : OVERLAP_EDGE_OPACITY, depthTest: false, depthWrite: false, blending: THREE.AdditiveBlending, toneMapped: false });
      ...
      const lineMaterial = new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: animated ? 0 : COVERAGE_EDGE_OPACITY, depthTest: false, depthWrite: false });
