...
function asVector(raDeg, decDeg, radius = SKY_RADIUS) {
...
export class SkyViewer {
    ...
    #camera = new THREE.PerspectiveCamera(45, 1, 0.01, 2.5);
    #renderer;
    ...
    #onSelection;
    #resizeObserver;
    #view = { raDeg: 180, decDeg: 0, fovDeg: 110 };
    ...
    #dragStart = null;
    #dragDistance = 0;
    constructor(canvas, onViewChange, onSelection) {
        this.#canvas = canvas;
    ...
    showCoverage(shape) {
        ...
        const line = new THREE.Line(geometry, new THREE.LineBasicMaterial({ color: 0x4ed6c8, transparent: true, opacity: 0.95 }));
        line.renderOrder = 4;
    ...
    showDensity(cells) {
        ...
        const edges = new THREE.LineSegments(edgeGeometry, new THREE.LineBasicMaterial({ color: 0xf0f4f5, transparent: true, opacity: 0.34, depthWrite: false }));
        edges.renderOrder = 4;
    ...
    #createStarField() {
        ...
        const geometry = new THREE.BufferGeometry();
        geometry.setAttribute("position", new THREE.BufferAttribute(positions, 3));
        return new THREE.Points(geometry, new THREE.PointsMaterial({ color: 0xa8b0bc, size: 1.05, sizeAttenuation: false, transparent: true, opacity: 0.28 }));
    }
    #createCoordinateGrid() {
        ...
        for (let ra = 0; ra < 360; ra += 30) {
            ...
            for (let dec = -90; dec <= 90; dec += 2)
                points.push(asVector(ra, dec, 1.004));
