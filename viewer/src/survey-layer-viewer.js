...
import { adjacentNeighbours, buildSurveyLayerModel, isAdjacentConnected, overlapCountByPixel, toggleConnectedRegion, visibleCoverageAtPixel, visibleSurveySlots, } from "../../src/survey-layer-model";
import { cartesianToRaDec } from "./coordinates";
import { buildSphericalCellGeometry, buildSphericalCellEdges, buildSphericalCellSheetGeometry, healpixPixelFromSceneDirection, sphericalCellBoundary, } from "./spherical-cell-geometry";
...
function fragmentMaterial(opacity = COVERAGE_OPACITY) {
...
export class SurveyLayerViewer {
    ...
    #camera = new THREE.PerspectiveCamera(48, 1, 0.015, 24);
    #renderer;
    #controls;
    #raycaster = new THREE.Raycaster();
    #pointer = new THREE.Vector2();
    #coverageGroup = new THREE.Group();
    #retiredGroup = new THREE.Group();
    ...
    #onStateChange;
    #model;
    #colorBySurvey = new Map();
    #selectedPixels = new Set();
    #visibleSurveyIds = new Set();
    #layerMeshes = [];
    #coverageEdgeMaterials = [];
    #meshBySurvey = new Map();
    #fragmentTransitions = [];
    #layoutMode = "layers";
    #interactionMode = "inspect";
    #focusedSurveyId = null;
    #pinnedCoveragePixel = null;
    #lockedCoveragePixel = null;
    #renderQueued = false;
    #pointerStart = null;
    #interactionMesh = null;
    #cameraTransition = null;
    #explosionTransition = null;
    #explodedFragments = [];
    #explodedPixel = null;
    constructor(canvas, manifest, surveys, onSelection, onHover, onInspection, onContextMenu, onStateChange) {
        this.#canvas = canvas;
    ...
    setVisibleSurveys(surveyIds) {
        ...
        if (next.size === this.#visibleSurveyIds.size && [...next].every((surveyId) => this.#visibleSurveyIds.has(surveyId)))
            return;
        this.#visibleSurveyIds.clear();
        this.#model.slots.forEach((slot) => {
            if (next.has(slot.surveyId))
                this.#visibleSurveyIds.add(slot.surveyId);
        });
        if (this.#focusedSurveyId && !this.#visibleSurveyIds.has(this.#focusedSurveyId))
            this.#focusedSurveyId = null;
        this.#pruneSelection();
        this.#rebuildVisible(this.#lockedCoveragePixel == null);
    }
    setInspectionLocked(pixel, locked) {
        if (!locked) {
            if (this.#lockedCoveragePixel === pixel)
    ...
    togglePinnedInspectionLock() {
        if (this.#interactionMode !== "inspect" || this.#pinnedCoveragePixel == null)
            return;
    ...
    setRegionSelection(pixels) {
        ...
        if (next.length > 1 && !isAdjacentConnected(this.#manifest.nside, next))
            throw new RangeError("Region selection must be eight-neighbour connected");
    ...
    focusSurvey(surveyId) {
        const pixels = this.#model.pixelsBySurvey.get(surveyId);
        if (!pixels?.length || !this.#visibleSurveyIds.has(surveyId))
            return;
    ...
    #rebuildVisible(animated) {
        ...
        const slots = visibleSurveySlots(this.#model, this.#visibleSurveyIds, this.#layoutMode);
        if (this.#layoutMode === "overlap")
            this.#buildOverlapLayer(animated);
        else
            slots.forEach((slot) => this.#buildSurveyLayer(slot.surveyId, slot.displayRadius, animated));
        ...
        if (this.#lockedCoveragePixel != null) {
            ...
            if (membership)
                this.#presentInspection(this.#lockedCoveragePixel, membership, this.#pixelDirection([this.#lockedCoveragePixel]), false);
        }
        this.#emitState();
        this.#requestRender();
    }
    #buildSurveyLayer(surveyId, radius, animated) {
        const pixels = this.#model.pixelsBySurvey.get(surveyId) ?? [];
        if (!pixels.length)
            return;
    ...
    #buildOverlapLayer(animated) {
        ...
        if (cells.length)
            this.#addFragmentLayer("__overlap__", pixels, cells, animated);
    }
    ...
    #addFragmentLayer(surveyId, pixels, cells, animated) {
        ...
        if (surveyId !== "__overlap__")
            this.#meshBySurvey.set(surveyId, mesh);
        if (animated) {
        ...
        for (const root of [...this.#coverageGroup.children]) {
            ...
            root.traverse((child) => {
                ...
                if (child instanceof THREE.LineSegments && child.material instanceof THREE.LineBasicMaterial)
                    lineMaterials.push(child.material);
            });
            this.#fragmentTransitions.push({
                root,
                startedAt: performance.now(),
    ...
    #pickCell(event) {
        ...
        const membership = visibleCoverageAtPixel(this.#model, pixel, this.#visibleSurveyIds);
        if (!membership && this.#interactionMode !== "region")
            return null;
        return { pixel, membership, point };
    }
    #intersectUnitSphere(ray) {
    ...
    #inspect(event) {
        ...
        if (this.#explodedPixel === hit.pixel) {
            if (this.#lockedCoveragePixel === hit.pixel)
                return;
            this.#pinnedCoveragePixel = null;
            this.#clearExplosion(true);
            this.#onInspection(null);
    ...
    #presentInspection(pixel, membership, point, rotateCamera) {
    ...
    #explodeInspection(pixel, membership, rotateCamera) {
        this.#clearExplosion(false);
        ...
        this.#coverageGroup.traverse((child) => {
            if (child instanceof THREE.Mesh && child.material instanceof THREE.MeshBasicMaterial)
                child.material.opacity = DIMMED_OPACITY;
            if (child instanceof THREE.LineSegments && child.material instanceof THREE.LineBasicMaterial)
                child.material.opacity = DIMMED_EDGE_OPACITY;
        });
        ...
        if (rotateCamera)
            this.#startCameraTransition(destination, new THREE.Vector3(0, 0, 0), 620);
        this.#requestRender();
    }
    #clearExplosion(animated) {
        ...
        this.#coverageGroup.traverse((child) => {
            if (child instanceof THREE.Mesh && child.material instanceof THREE.MeshBasicMaterial)
                child.material.opacity = COVERAGE_OPACITY;
            if (child instanceof THREE.LineSegments && child.material instanceof THREE.LineBasicMaterial)
                child.material.opacity = COVERAGE_EDGE_OPACITY;
        });
    ...
    #startCameraTransition(destination, target, durationMs) {
    ...
    #advanceFragmentTransitions(now) {
    ...
    #advanceExplosionTransition(now) {
    ...
    #keepCameraOutside() {
        ...
        if (this.#camera.position.length() >= minimumRadius)
            return;
        if (this.#camera.position.lengthSq() === 0)
            this.#camera.position.set(minimumRadius, 0, 0);
        else
            this.#camera.position.setLength(minimumRadius);
    }
    ...
    #handlePointerLeave = () => this.#onHover(null);
    #handlePointerCancel = () => { this.#pointerStart = null; };
    #handleContextMenu = (event) => {
        event.preventDefault();
        const hit = this.#pickCell(event);
        this.#onContextMenu(hit?.membership ? {
            clientX: event.clientX,
            clientY: event.clientY,
            pixel: hit.pixel,
    ...
    #handleKeyDown = (event) => {
        if (event.key !== "Escape" || this.#explodedPixel == null)
            return;
        this.#pinnedCoveragePixel = null;
        this.#lockedCoveragePixel = null;
        this.#clearExplosion(true);
    ...
    #requestRender() {
        ...
        requestAnimationFrame((now) => {
            ...
            if (moving || this.#cameraTransition || this.#fragmentTransitions.length || this.#explosionTransition)
                this.#requestRender();
        });
    }
}
